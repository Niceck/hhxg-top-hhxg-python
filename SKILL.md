...
- "今天 A 股怎么样" / "大盘怎么样" / "盘后复盘" → fetch_snapshot.py
- "热门题材" / "连板天梯" / "龙虎榜" / "赚钱效应" → fetch_snapshot.py
- "今天是交易日吗" / "下周解禁" / "交割日" → calendar.py
- "融资融券" / "两融数据" / "融资净买入" → margin.py
- "量化选股" / "选股策略" / "游资席位" → strategy.py
- "最新快讯" / "财经新闻" / "焦点新闻" → news.py
- "今日选股" / "九转信号" / "多因子" / "量化信号" → fetch_snapshot.py signals
...
6. **非交易日提示** — 周末或节假日用户问行情时，先说"今天休市"，然后展示最近一个交易日的数据
